...
<?php 
$itemid=$_GET["id"];
$query = "SELECT ".$prefix."category.category_name, brand, model, ".$prefix."player.player_name, ".$prefix."category.category_img, kaucja, koszt_za_dzien, ID_itemstatus FROM ((".$prefix."items
...
$result = mysqli_query ($link, $query) or die ("Zapytanie zakończone niepowodzeniem");
    ...
    $statusPrzedmiotu = $row['ID_itemstatus'];
...
$result = mysqli_query ($link, $query) or die ("Zapytanie zakończone niepowodzeniem");
  ...
  {
    echo "<a class=\"btn btn-primary\" href=\"../edit/edit.php?itemId=".$itemid."\">Edytuj</a>";
  }
  ...
  break;
  case 2:
    echo "<p> Przedmiot jest chwilowo niedostępny </p>";
    echo " </div> </div></div> </div></div>";
    break;
}
}
...
function worker_rent_for_client()
{ $itemid=$_GET["id"];
  echo "<form class=\"form-horizontal\" method=\"POST\" action=\"rent.php\">
          <div class=\"container px-lg-5\">
